...
def db_session():
    ...
    if not url:
        pytest.skip(
            ...
            "to run PostgreSQL Career Vault tests."
        )
...
def test_update_existing_user_career_profile(db_session):
    session, created_user_ids = db_session
    repository = CareerProfileRepository(session)

    original_profile = rich_profile()

    user_id = repository.save(original_profile)
    created_user_ids.append(user_id)
    ...
    updated_profile.personal_information.full_name = "Ada Byron"
    updated_profile.personal_information.phone = "555-0200"
    updated_profile.personal_information.summary = (
        "Updated computing pioneer."
    )

    updated_profile.skills.technical = [
        "Python",
        "FastAPI",
        "PostgreSQL",
    ]

    updated_profile.projects[0].description = (
        "Updated resume analysis platform."
    )

    returned_user_id = repository.save(
        updated_profile,
        user_id=user_id,
    )
    ...
    assert retrieved is not None
    assert retrieved.personal_information.full_name == "Ada Byron"
    assert retrieved.personal_information.phone == "555-0200"
    assert (
        retrieved.personal_information.summary
        == "Updated computing pioneer."
    )

    assert retrieved.skills.technical == [
        "Python",
        "FastAPI",
        "PostgreSQL",
    ]
    ...
    assert (
        retrieved.projects[0].description
        == "Updated resume analysis platform."
    )
...
@pytest.mark.asyncio
async def test_resume_analysis_persists_validated_profile(
    db_session,
):
    session, created_user_ids = db_session
...
@pytest.mark.asyncio
async def test_invalid_career_profile_is_not_persisted(
    db_session,
):
    session, _ = db_session
    ...
    use_case = AnalyzeResumeUseCase(
        DocumentExtractionService(),
        InvalidLLM(),
        career_profile_repository=CareerProfileRepository(
            session
        ),
    )
